...
class Dataset(object):
    ...
    def _check_saved_data(self, splitter, ori_prefix, saved_prefix):
        ...
        if splitter in ("loo", "ratio"):
            ...
            valid_file = ori_prefix + ".valid"
            ori_file_md5 = [check_md5(file) for file in [train_file, test_file, valid_file]]
        else:
        ...
        # check saved files
        for postfix in [".train", ".test", ".valid", ".user2id", ".item2id"]:
    ...
    def _load_data(self, config):
        ...
        valid_file = saved_prefix + ".valid"
        user_map_file = saved_prefix + ".user2id"
        item_map_file = saved_prefix + ".item2id"

        if False and self._check_saved_data(splitter, ori_prefix, saved_prefix):
            ...
            valid_data = pd.read_csv(valid_file, sep=sep, header=None, names=columns)
            ...
            train_data, test_data, valid_data = self._split_data(ori_prefix, saved_prefix, columns, by_time, config)
        ...
        if file_format == "UI":
            ...
            valid_ratings = [1.0] * len(valid_data["user"])
        else:
        ...
        self.test_matrix = csr_matrix((test_ratings, (test_data["user"], test_data["item"])),
                                      shape=(self.num_users, self.num_items))
        self.valid_matrix = csr_matrix((valid_ratings, (valid_data["user"], valid_data["item"])),
                                      shape=(self.num_users, self.num_items))

        self.trainUser = train_data["user"]
        self.trainItem = train_data["item"]
        self.trainDataSize = len(self.trainUser)
        if self.conf["with_item_vat"]:
            if self.dataset_name == "tiktok":
                self.v_feat = torch.load("{}/{}_visual_feat.pt".format(self.conf["data.input.path"],self.dataset_name))[list(self.itemids.keys())]
                self.a_feat = torch.load("{}/{}_audio_feat.pt".format(self.conf["data.input.path"],self.dataset_name))[list(self.itemids.keys())]
                t_feat_origin = torch.load("{}/{}_textual_feat.pt".format(self.conf["data.input.path"],self.dataset_name)).detach()
                # remap id
                remaped_word_tensor=[]
                for i in t_feat_origin.T:
                    idx = i[0].cpu().item()
                    if idx in self.itemids:
                        remaped_word_tensor.append([self.itemids[idx],i[1]])
                self.words_tensor = torch.tensor(remaped_word_tensor).T
#                 self.word_embedding = torch.nn.Embedding(11574, 128).cuda()
#                 torch.nn.init.xavier_normal_(self.word_embedding.weight).cuda()
#                 self.t_feat = scatter(self.word_embedding(self.words_tensor[1]),self.words_tensor[0],reduce='mean',dim=0).cuda()
                ...
                # only one modality(visual feature) in Kwai dataset.
                self.v_feat = torch.load("{}/{}_feat_v.pt".format(self.conf["data.input.path"],self.dataset_name))[list(self.itemids.keys())]
            else:
                print("Load Item's Features(Visual, Acoustic, Textual)")
                self.v_feat = torch.from_numpy(np.load("{}/{}_FeatureVideo_normal.npy".format(self.conf["data.input.path"],self.dataset_name))[list(self.itemids.keys())])
                self.a_feat = torch.from_numpy(np.load("{}/{}_FeatureAudio_avg_normal.npy".format(self.conf["data.input.path"],self.dataset_name))[list(self.itemids.keys())])
                self.t_feat = torch.from_numpy(np.load("{}/{}_FeatureText_stl_normal.npy".format(self.conf["data.input.path"],self.dataset_name))[list(self.itemids.keys())])
    ...
    def _split_data(self, ori_prefix, saved_prefix, columns, by_time, config):
        ...
        if splitter in ("loo", "ratio"):
            raise NotImplementedError("'%s' is not supported!" % splitter)
        elif splitter == "given":
            train_file = ori_prefix + ".train"
            test_file = ori_prefix + ".test"
            valid_file = ori_prefix + ".valid"
            ...
            with open(saved_prefix+".md5", "w") as md5_out:
                md5_out.writelines('\n'.join([check_md5(train_file), check_md5(test_file), check_md5(valid_file)]))
        else:
            raise ValueError("'%s' is an invalid splitter!" % splitter)
        ...
        valid_data["user"] = valid_data["user"].map(self.userids)

        unique_item = all_data["item"].unique()
        # print("item",unique_item.shape)
        ...
        all_remapped_data = pd.concat([train_data, test_data, valid_data])
    ...
    def get_user_valid_dict(self):
        valid_dict = csr_to_user_dict(self.valid_matrix)
        return valid_dict
    ...
    def get_user_valid_neg_dict(self):
        valid_neg_dict = None
        if self.negative_matrix is not None:
            valid_neg_dict = csr_to_user_dict(self.negative_matrix)
        return valid_neg_dict
    ...
    def get_test_interactions(self):
        dok_matrix = self.test_matrix.todok()
        users_list, items_list = [], []
        for (user, item), value in dok_matrix.items():
            users_list.append(user)
            items_list.append(item)
    ...
    def get_valid_interactions(self):
        dok_matrix = self.valid_matrix.todok()
        users_list, items_list = [], []
        for (user, item), value in dok_matrix.items():
            users_list.append(user)
            items_list.append(item)
