...
class BasicModel(nn.Module):
    def __init__(self,dataset, config):
        ...
        self.valid_evaluator = ProxyEvaluator(dataset,
                                        dataset.get_user_train_dict(),
                                        dataset.get_user_valid_dict(),
                                        None,
                                        metric=self.config["metric"],
                                        group_view=self.config["group_view"],
                                        top_k=self.config["topks"],
                                        batch_size=self.config["test_batch_size"],
                                        num_thread=self.config["num_thread"])
        self.test_evaluator = ProxyEvaluator(dataset,
                                        dataset.get_user_train_dict(),
    ...
    def compute(self):
        raise NotImplementedError
    def evaluate(self):
        return self.valid_evaluator.evaluate(self)
    def test(self):
        return self.test_evaluator.evaluate(self)
